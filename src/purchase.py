...
def create_cart(new_cart: Customer):
    ...
    with db.engine.begin() as conn:
        ...
        cartID = conn.execute(sqlalchemy.text("INSERT INTO carts (user_id) VALUES (:id) RETURNING cart_id"), {"id": id}).scalar()
...
def add_to_cart(cart_id: int, character_name: str):
    """ """
    with db.engine.begin() as conn:
        userID = conn.execute(sqlalchemy.text("SELECT user_id FROM carts WHERE cart_id = :cartID"), [{"cartID": cart_id}]).scalar()
        characterID = conn.execute(sqlalchemy.text("SELECT character_id FROM characters WHERE name = :Name"), {"Name": character_name})
